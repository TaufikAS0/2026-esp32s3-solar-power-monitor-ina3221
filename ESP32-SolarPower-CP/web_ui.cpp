...
<body>
  ...
  <div class="card chart-card">
    <div class="chart-toolbar">
      ...
      <div class="chart-view-switch">
        ...
        <button class="chart-mode-btn" id="history-reset-zoom" type="button">Reset Zoom</button>
  ...
  <script>
    let maxPts=300,chartReady=false,lastUpdate=Date.now(),pollMs=1000,pollTimer=null,rawPollMs=250,rawPollTimer=null,rawFetchBusy=false,rawCursorSeq=0,batteryPresets=[],serverFormDirty=false,serverFormSaving=false,flowFormDirty=false,flowFormSaving=false,ledFormDirty=false,ledFormSaving=false,ledLiveTimer=null,ledSaveTimer=null,historyScale='1s',historyCapacityMax=600,runtimeReportMs=1000,runtimeSensorPollMs=1,runtimeRawEspCapacity=2048,runtimeRawFetchLimit=256,runtimeMeasuredHz=0,runtimeInaAvg=1,viewFollowLive=true,viewStartMs=0,viewEndMs=0,yAxisLocked=false;const charts={},trendHistory=[],rawHistory=[],SOLAR='#f59e0b',BAT='#10b981',BAT_DIS='#ef4444',COMBO='#a855f7',MUTED='#4a6080',WARN='#ef4444',LOAD='#38bdf8',TREND_HISTORY_LIMIT=3600,RAW_HISTORY_KEEP_MS=600000,HISTORY_SCALE_KEY='solarMonitorHistoryScaleV1',HISTORY_SCALES={s1:{id:'1s',label:'1s',gridMs:1000,windowMs:10000,source:'raw'},s3:{id:'3s',label:'3s',gridMs:3000,windowMs:30000,source:'raw'},s10:{id:'10s',label:'10s',gridMs:10000,windowMs:100000,source:'raw'},m1:{id:'1min',label:'1min',gridMs:60000,windowMs:600000,source:'trend'}},yAxisRanges={power:null,voltage:null,current:null},lastRendered={points:[],viewStartMs:0,viewEndMs:0,sourceMode:'raw',scaleId:'1s'},zoomSelection={active:false,chartId:'',startPx:0,endPx:0};
    ...
    function visibleWindowPointEstimate(scale){if(scale.source==='raw'){const hz=runtimeMeasuredHz>0?runtimeMeasuredHz:(1000/Math.max(1,runtimeSensorPollMs));return Math.max(1,Math.round(scale.windowMs*hz/1000))}return Math.max(1,Math.round(scale.windowMs/Math.max(1,runtimeReportMs)))}
    ...
    function clearBrowserHistory(){trendHistory.length=0;rawHistory.length=0;rawCursorSeq=0;viewFollowLive=true;viewStartMs=0;viewEndMs=0;renderHistoryFromBrowser('Browser RAM cleared. Waiting for new live samples...')}
    function resetHistoryZoom(note){viewFollowLive=true;viewStartMs=0;viewEndMs=0;renderHistoryFromBrowser(note||'Zoom reset to live follow.')}
    ...
    function canvasPixelX(ev){const canvas=ev.currentTarget,rect=canvas.getBoundingClientRect(),rawX=(ev.clientX-rect.left)*(canvas.width/Math.max(1,rect.width));return Math.max(0,Math.min(canvas.width,rawX))}
    ...
    function beginChartSelection(ev){zoomSelection.active=true;zoomSelection.chartId=ev.currentTarget.id;zoomSelection.startPx=canvasPixelX(ev);zoomSelection.endPx=zoomSelection.startPx;const chart=chartByCanvasId(zoomSelection.chartId);if(chart)chart.draw()}
    ...
    function installChartInteractions(){['powerChart','voltageChart','currentChart'].forEach(id=>{const canvas=document.getElementById(id);if(!canvas||canvas.dataset.zoomReady==='1')return;canvas.dataset.zoomReady='1';canvas.addEventListener('mousedown',beginChartSelection);canvas.addEventListener('mousemove',moveChartSelection);canvas.addEventListener('dblclick',()=>resetHistoryZoom('Zoom reset from chart double-click.'))});if(!window.__solarChartSelectionHooked){window.__solarChartSelectionHooked=true;window.addEventListener('mouseup',endChartSelection)}}
    function sourceModeLabel(mode){if(mode==='trend-fallback')return'trend fallback';if(mode==='raw')return'raw live';return'trend'}
    ...
    function chooseHistorySource(scale,startMs,endMs){if(scale.source==='trend'){if(trendHistory.length)return{points:trendHistory,mode:'trend'};if(rawHistory.length)return{points:rawHistory,mode:'raw'};return{points:[],mode:'trend'}}if(startMs===0&&endMs===0)return rawHistory.length?{points:rawHistory,mode:'raw'}:(trendHistory.length?{points:trendHistory,mode:'trend-fallback'}:{points:[],mode:'raw'});if(rawSourceCovers(startMs,endMs))return{points:rawHistory,mode:'raw'};if(trendHistory.length)return{points:trendHistory,mode:'trend-fallback'};return rawHistory.length?{points:rawHistory,mode:'raw'}:{points:[],mode:'raw'}}
    ...
    function stableBucketSamples(points,bucketMs){if(!bucketMs||bucketMs<=1||points.length<=2)return points.slice();const reduced=[];let activeBucket=-1,lastPoint=null;for(let index=0;index<points.length;index++){const point=points[index],bucket=Math.floor((Number(point.t)||0)/bucketMs);if(bucket!==activeBucket){if(lastPoint)reduced.push(lastPoint);activeBucket=bucket}lastPoint=point}if(lastPoint)reduced.push(lastPoint);return reduced}
    function displaySamplesForScale(points,scale,sourceMode){if(sourceMode!=='raw')return points.length>maxPts?downsampleBrowserSamples(points,maxPts):points.slice();if(scale.id==='10s')return stableBucketSamples(points,10);return points.slice()}
    function renderHistoryFromBrowser(noteOverride){const scale=activeHistoryScale(),preferredSource=chooseHistorySource(scale,viewFollowLive?0:viewStartMs,viewFollowLive?0:viewEndMs),baseSource=preferredSource.points;if(!baseSource.length){syncCharts([],Date.now(),scale);renderHistoryModeSummary(noteOverride||(scale.source==='raw'?'Waiting for raw live stream...':'Waiting for trend snapshots...'));return}const viewRange=effectiveViewRange(scale,baseSource),resolvedSource=chooseHistorySource(scale,viewRange.startMs,viewRange.endMs),source=resolvedSource.points;if(!source.length){syncCharts([],Date.now(),scale);renderHistoryModeSummary(noteOverride||'Waiting for browser history...');return}const clampedRange=effectiveViewRange(scale,source),startIndex=lowerBoundByTime(source,clampedRange.startMs),endIndex=lowerBoundByTime(source,clampedRange.endMs+1),visible=(startIndex<endIndex?source.slice(startIndex,endIndex):source.slice(Math.max(0,source.length-1))),displayPoints=displaySamplesForScale(visible,scale,resolvedSource.mode),viewEndMsLocal=clampedRange.endMs||Number(source[source.length-1].t)||Date.now(),visibleCount=visible.length;lastRendered.points=displayPoints.slice();lastRendered.viewStartMs=clampedRange.startMs;lastRendered.viewEndMs=viewEndMsLocal;lastRendered.sourceMode=resolvedSource.mode;lastRendered.scaleId=scale.id;syncCharts(displayPoints,viewEndMsLocal,scale);const note=noteOverride||('Grid '+scale.label+' | '+sourceModeLabel(resolvedSource.mode)+' | '+visibleCount+' pts'+(visibleCount!==displayPoints.length?(' -> '+displayPoints.length+' chart pts'):'')+(viewFollowLive?' | live tail':' | zoom '+formatHistorySpan(clampedRange.endMs-clampedRange.startMs))+(resolvedSource.mode==='raw'?(' | avg x'+runtimeInaAvg+' | '+(runtimeMeasuredHz>0?runtimeMeasuredHz.toFixed(1):'0.0')+' Hz'):' | older area shown from browser trend cache'));renderHistoryModeSummary(note)}
    function renderHistoryModeSummary(noteOverride){const scale=activeHistoryScale(),rawSpanMs=rawBrowserHistorySpanMs(),rangeLabel=!viewFollowLive&&viewEndMs>viewStartMs?(' | Zoom '+formatHistorySpan(viewEndMs-viewStartMs)):' | Live tail';document.querySelectorAll('[data-history-scale]').forEach(btn=>btn.classList.toggle('active',btn.dataset.historyScale===historyScale));document.getElementById('history-mode-note').textContent='Grid '+scale.label+' | '+sourceModeLabel(lastRendered.sourceMode)+' | Window '+formatHistorySpan(scale.windowMs)+' | ~'+visibleWindowPointEstimate(scale)+' pts'+rangeLabel;document.getElementById('history-source-note').textContent=noteOverride||('Raw RAM '+formatHistorySpan(rawSpanMs)+' | Status '+runtimeReportMs+' ms | Fetch '+rawPollMs+' ms | avg x'+runtimeInaAvg+' | drag on chart to zoom | double-click or Reset Zoom to follow live')}
    function setHistoryScale(view){historyScale=(view==='3s'||view==='10s'||view==='1min')?view:'1s';if(!viewFollowLive&&viewEndMs>viewStartMs){const scale=activeHistoryScale(),centerMs=(viewStartMs+viewEndMs)/2;viewStartMs=centerMs-(scale.windowMs/2);viewEndMs=centerMs+(scale.windowMs/2)}saveHistoryScale();renderHistoryFromBrowser()}
    ...
    function syncOne(ch,points,latestTimestampMs,keyA,keyB,keyC,scale,axisKey){if(!ch)return;const pointRadius=scale.source==='raw'?(points.length>1200?0.5:points.length>600?0.75:1):(points.length>600?0.5:1.1);ch.data.datasets[0].data=pointSeries(points,latestTimestampMs,keyA);ch.data.datasets[1].data=pointSeries(points,latestTimestampMs,keyB);ch.data.datasets[2].data=pointSeries(points,latestTimestampMs,keyC);ch.data.datasets.forEach(ds=>{ds.pointRadius=pointRadius;ds.pointHoverRadius=Math.max(3,pointRadius*4);ds.pointHitRadius=8});ch.options.scales.x.min=(lastRendered.viewStartMs-latestTimestampMs)/1000;ch.options.scales.x.max=0;ch.options.scales.x.ticks.stepSize=scale.gridMs/1000;applyYAxisRange(ch,axisKey);ch.update('none')}
    ...
    async function fetchRawHistory(){if(rawFetchBusy)return;rawFetchBusy=true;let statusNote='';try{let loops=0;while(loops<6){const ctrl=new AbortController(),tid=setTimeout(()=>ctrl.abort(),2000),url='/api/history/live?after_seq='+rawCursorSeq+'&limit='+runtimeRawFetchLimit,r=await fetch(url,{signal:ctrl.signal});clearTimeout(tid);const data=await r.json();if(data&&data.overflowed){rawHistory.length=0;statusNote='Raw buffer rolled over; showing newest burst.'}const appended=appendRawHistoryBatch(Array.isArray(data&&data.points)?data.points:[]);if(appended===0&&!data.truncated)break;if(!data.truncated)break;loops++}if(activeHistoryScale().source==='raw'||statusNote)renderHistoryFromBrowser(statusNote||undefined)}catch(e){}finally{rawFetchBusy=false;scheduleRawPoll()}}
    ...
    window.addEventListener('load',()=>{
      ...
      waitForCharts();
      fetchConfig();
      renderHistoryFromBrowser('Waiting for browser RAM live history...');
      fetchRawHistory();
      fetchData();
      ...
      document.getElementById('history-reset-zoom').onclick=()=>resetHistoryZoom();
