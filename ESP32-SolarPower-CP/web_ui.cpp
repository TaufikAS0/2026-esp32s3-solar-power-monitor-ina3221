...
const char* historySampleNote(HistoryViewMode viewMode) {
  return viewMode == HistoryViewMode::Minutes
             ? "No chart averaging. Long archive stores fixed 10 s sensor snapshots in FIFO order."
...
<body>
  ...
  <script>
    ...
    function sourceModeLabel(mode){if(mode==='raw-live')return'live burst';if(mode==='raw-cache')return'browser raw cache';if(mode==='archive-cache')return'device archive';return'browser archive'}
    ...
    function stableVisibleHistory(points,limit){if(points.length<=limit||limit<3)return points.slice();const firstMs=Number(points[0].t)||0,lastMs=Number(points[points.length-1].t)||firstMs,spanMs=Math.max(1,lastMs-firstMs),bucketMs=Math.max(1,Math.ceil(spanMs/Math.max(2,limit-1)));return stableBucketSamples(points,bucketMs)}
    function displaySamplesForScale(points,scale,sourceMode){if(!sourceUsesRaw(sourceMode)){if(scale.id==='1min'||scale.id==='10min')return points.slice();return points.length>maxPts?downsampleBrowserSamples(points,maxPts):points.slice()}if(scale.id==='1s')return points.slice();return stableVisibleHistory(points,maxPts)}
