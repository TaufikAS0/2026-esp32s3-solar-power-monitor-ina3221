...
<body>
  ...
  <script>
    let maxPts=300,chartReady=false,lastUpdate=Date.now(),pollMs=1000,pollTimer=null,rawPollMs=250,rawPollTimer=null,rawFetchBusy=false,rawCursorSeq=0,batteryPresets=[],serverFormDirty=false,serverFormSaving=false,flowFormDirty=false,flowFormSaving=false,ledFormDirty=false,ledFormSaving=false,ledLiveTimer=null,ledSaveTimer=null,historyScale='1s',historyCapacityMax=600,runtimeReportMs=1000,runtimeSensorPollMs=1,runtimeRawEspCapacity=2048,runtimeRawFetchLimit=256,runtimeMeasuredHz=0,runtimeInaAvg=1;const charts={},trendHistory=[],rawHistory=[],SOLAR='#f59e0b',BAT='#10b981',BAT_DIS='#ef4444',COMBO='#a855f7',MUTED='#4a6080',WARN='#ef4444',LOAD='#38bdf8',TREND_HISTORY_LIMIT=3600,RAW_HISTORY_KEEP_MS=120000,HISTORY_SCALE_KEY='solarMonitorHistoryScaleV1',HISTORY_SCALES={s1:{id:'1s',label:'1s',gridMs:1000,windowMs:10000,source:'raw'},s3:{id:'3s',label:'3s',gridMs:3000,windowMs:30000,source:'raw'},s10:{id:'10s',label:'10s',gridMs:10000,windowMs:100000,source:'raw'},m1:{id:'1min',label:'1min',gridMs:60000,windowMs:600000,source:'trend'}};
    ...
    function chartLimitForScale(scale){if(scale.source!=='raw')return maxPts;if(scale.id==='1s')return Math.max(maxPts,10000);if(scale.id==='3s')return Math.max(maxPts,6000);return maxPts}
    ...
    function pruneTimedHistory(list,minTimestampMs){let cut=0;while(cut<list.length&&(Number(list[cut].t)||0)<minTimestampMs)cut++;if(cut>0)list.splice(0,cut)}
    function clearBrowserHistory(){trendHistory.length=0;rawHistory.length=0;rawCursorSeq=0;renderHistoryFromBrowser('Browser RAM cleared. Waiting for new live samples...')}
    ...
    function appendRawHistoryBatch(points){let appended=0;const currentScale=activeHistoryScale();points.forEach(item=>{if(!Array.isArray(item)||item.length<11)return;const seq=Number(item[0])||0;if(seq<=rawCursorSeq)return;rawHistory.push({seq:seq,t:Number(item[1])||Date.now(),ps:Number(item[2])||0,pb:Number(item[3])||0,pl:Number(item[4])||0,vs:Number(item[5])||0,vb:Number(item[6])||0,vl:Number(item[7])||0,is:Number(item[8])||0,ib:Number(item[9])||0,il:Number(item[10])||0});rawCursorSeq=seq;appended++});if(rawHistory.length){const latestTimestampMs=Number(rawHistory[rawHistory.length-1].t)||Date.now(),minimumWindowMs=Math.max(RAW_HISTORY_KEEP_MS,currentScale.source==='raw'?currentScale.windowMs:0);pruneTimedHistory(rawHistory,latestTimestampMs-minimumWindowMs)}return appended}
    ...
    function renderHistoryFromBrowser(noteOverride){const scale=activeHistoryScale(),source=scale.source==='raw'?rawHistory.slice():trendHistory.slice();if(!source.length){syncCharts([],Date.now(),scale);renderHistoryModeSummary(noteOverride||(scale.source==='raw'?'Waiting for raw live stream...':'Waiting for trend snapshots...'));return}const latestTimestampMs=Number(source[source.length-1].t)||Date.now(),windowStartMs=latestTimestampMs-scale.windowMs;let visible=source.filter(p=>(Number(p.t)||0)>=windowStartMs);if(!visible.length)visible=source.slice(-1);const visibleCount=visible.length,chartLimit=chartLimitForScale(scale);if(visible.length>chartLimit)visible=downsampleBrowserSamples(visible,chartLimit);syncCharts(visible,latestTimestampMs,scale);const note=noteOverride||((scale.source==='raw'?'Raw':'Trend')+' '+scale.label+' | '+visibleCount+' pts'+(visibleCount!==visible.length?(' -> '+visible.length+' chart pts'):'')+(scale.source==='raw'?(' | avg x'+runtimeInaAvg+' | '+(runtimeMeasuredHz>0?runtimeMeasuredHz.toFixed(1):'0.0')+' Hz'):''));renderHistoryModeSummary(note)}
    function renderHistoryModeSummary(noteOverride){const scale=activeHistoryScale(),rawSpanMs=rawBrowserHistorySpanMs(),sourceLabel=scale.source==='raw'?'raw live':'trend';document.querySelectorAll('[data-history-scale]').forEach(btn=>btn.classList.toggle('active',btn.dataset.historyScale===historyScale));document.getElementById('history-mode-note').textContent='Grid '+scale.label+' | '+sourceLabel+' | Window '+formatHistorySpan(scale.windowMs)+' | ~'+visibleWindowPointEstimate(scale)+' pts';document.getElementById('history-source-note').textContent=noteOverride||('Raw RAM '+formatHistorySpan(rawSpanMs)+' | Status '+runtimeReportMs+' ms | Fetch '+rawPollMs+' ms | avg x'+runtimeInaAvg+' | '+(runtimeMeasuredHz>0?runtimeMeasuredHz.toFixed(1):'0.0')+' Hz | reload clears local history')}
    ...
    function syncOne(ch,points,latestTimestampMs,keyA,keyB,keyC,scale){if(!ch)return;const pointRadius=scale.source==='raw'?(points.length>1200?0.5:points.length>600?0.75:1):(points.length>600?0.5:1.1);ch.data.datasets[0].data=pointSeries(points,latestTimestampMs,keyA);ch.data.datasets[1].data=pointSeries(points,latestTimestampMs,keyB);ch.data.datasets[2].data=pointSeries(points,latestTimestampMs,keyC);ch.data.datasets.forEach(ds=>{ds.pointRadius=pointRadius;ds.pointHoverRadius=Math.max(3,pointRadius*4);ds.pointHitRadius=8});ch.options.scales.x.min=-(scale.windowMs/1000);ch.options.scales.x.max=0;ch.options.scales.x.ticks.stepSize=scale.gridMs/1000;ch.update('none')}
...
void WebUi::handleLiveHistory_() {
  ...
  body += F("\"sample_note\":\"ESP keeps short raw burst; browser keeps long history.\",");
  body += F("\"after_seq\":");
  ...
  for (size_t index = 0; index < availableCount; ++index) {
    ...
    body += ',';
    body += jsonNumber(point.solarPowerMw);
    body += ',';
    body += jsonNumber(point.batteryPowerSignedMw);
    body += ',';
    body += jsonNumber(point.loadPowerMw);
    body += ',';
