...
<body>
  ...
  <script>
    ...
    function displaySamplesForScale(points,scale){if(scale.source!=='raw'){return points.length>maxPts?downsampleBrowserSamples(points,maxPts):points.slice()}if(scale.id==='10s')return stableBucketSamples(points,10);return points.slice()}
    function renderHistoryFromBrowser(noteOverride){const scale=activeHistoryScale(),source=scale.source==='raw'?rawHistory:trendHistory;if(!source.length){syncCharts([],Date.now(),scale);renderHistoryModeSummary(noteOverride||(scale.source==='raw'?'Waiting for raw live stream...':'Waiting for trend snapshots...'));return}const latestTimestampMs=Number(source[source.length-1].t)||Date.now(),windowStartMs=latestTimestampMs-scale.windowMs,startIndex=lowerBoundByTime(source,windowStartMs);let visible=startIndex<source.length?source.slice(startIndex):source.slice(-1);if(!visible.length)visible=source.slice(-1);const visibleCount=visible.length,displayPoints=displaySamplesForScale(visible,scale);syncCharts(displayPoints,latestTimestampMs,scale);const note=noteOverride||((scale.source==='raw'?'Raw':'Trend')+' '+scale.label+' | '+visibleCount+' pts'+(visibleCount!==displayPoints.length?(' -> '+displayPoints.length+' chart pts'):'')+(scale.source==='raw'?(' | fixed browser cache | avg x'+runtimeInaAvg+' | '+(runtimeMeasuredHz>0?runtimeMeasuredHz.toFixed(1):'0.0')+' Hz'):''));renderHistoryModeSummary(note)}
