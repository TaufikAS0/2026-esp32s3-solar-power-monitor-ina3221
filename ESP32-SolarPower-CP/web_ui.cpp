...
<body>
  ...
  <div class="cfg-grid">
    ...
    <div class="card">
      <div class="lbl">Runtime Config</div>
      <div class="sub" style="margin-bottom:10px">Unified interval for sensor read, history sample, and dashboard refresh.</div>
      <div style="display:flex;gap:6px;align-items:center;margin-bottom:8px">
        <input class="inp" id="cfg-interval" type="number" value="1000" min="200" max="60000" style="flex:1">
        <span class="sub">ms</span>
      </div>
      <button class="btn-primary" id="apply-interval" style="width:100%">Apply Sample Interval</button>
  ...
  <script>
    ...
    async function fetchHistory(){try{const r=await fetch('/api/history');const d=await r.json();resetHist();(d.points||[]).slice(-MAX_PTS).forEach(p=>appendPoint({ps:p.p_solar_mw,pb:p.p_bat_signed_mw,pl:p.p_load_mw,vs:p.v_solar_v,vb:p.v_bat_v,vl:p.v_load_v,is:p.i_solar_ma,ib:p.i_bat_ma,il:p.i_load_ma}));syncCharts()}catch(e){}}
    ...
    async function fetchConfig(){
      try{
        ...
        document.getElementById('cfg-interval').value=c.sample_interval_ms||1000;
        ...
        document.getElementById('interval-note').textContent='Active interval: '+(c.sample_interval_ms||1000)+' ms';
        ...
        document.getElementById('hdr-interval').textContent=(c.sample_interval_ms||1000)+' ms';
    ...
    async function fetchData(){
      try{
        ...
        document.getElementById('hdr-interval').textContent=(d.sample_interval_ms||pollMs)+' ms';
        ...
        document.getElementById('interval-note').textContent='Active interval: '+(d.sample_interval_ms||pollMs)+' ms';
    ...
    async function saveRuntimeConfig(){const body=new URLSearchParams();body.set('sample_interval_ms',document.getElementById('cfg-interval').value);const r=await fetch('/api/config/runtime',{method:'POST',body});const d=await r.json();alert(d.message||'Runtime updated');fetchConfig();fetchData()}
...
void WebUi::handleSaveRuntime_() {
  const String rawValue = server_.arg("sample_interval_ms");
  uint32_t requestedInterval =
      rawValue.isEmpty() ? wifiService_.config().sampleIntervalMs : rawValue.toInt();
  if (requestedInterval == 0) {
    requestedInterval = Config::kDefaultSampleIntervalMs;
  }
  wifiService_.saveRuntimeConfig(requestedInterval);
  sensors_.setSampleIntervalMs(wifiService_.config().sampleIntervalMs);

  String body;
  body.reserve(96);
  body += F("{\"ok\":true,\"message\":\"Sample interval applied\",\"sample_interval_ms\":");
  body += String(wifiService_.config().sampleIntervalMs);
  body += '}';
  server_.send(200, "application/json", body);
}
