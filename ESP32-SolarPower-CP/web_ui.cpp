...
const char* historySampleNote(HistoryViewMode viewMode) {
  return viewMode == HistoryViewMode::Minutes
             ? "No chart averaging. Minute view stores 1 s sensor snapshots for a longer trend."
...
<body>
  ...
  <div class="card chart-card">
    <div class="chart-toolbar">
      <div class="chart-meta">
        ...
        <div class="sub" id="history-mode-note">Second view: waiting for runtime config...</div>
      ...
      <div class="chart-view-switch">
        <button class="chart-mode-btn active" id="history-view-seconds" type="button" data-history-view="seconds">Seconds</button>
        <button class="chart-mode-btn" id="history-view-minutes" type="button" data-history-view="minutes">Minutes</button>
  ...
  <div class="card chart-card">
    ...
    <div style="height:180px;margin-top:10px"><canvas id="powerChart"></canvas></div>
  </div>

  <div class="card chart-card">
    <div class="lbl">Voltage History</div>
    <div style="height:180px;margin-top:10px"><canvas id="voltageChart"></canvas></div>
  </div>

  <div class="card chart-card">
    <div class="lbl">Current History</div>
    <div style="height:180px;margin-top:10px"><canvas id="currentChart"></canvas></div>
  </div>
  ...
  <div class="cfg-grid">
    ...
    <div class="card">
      ...
      <div style="display:flex;gap:6px;align-items:center;margin-bottom:10px">
        <input class="inp" id="cfg-chart-points" type="number" value="180" min="30" max="300" style="flex:1">
  ...
  <script>
    let maxPts=300,chartReady=false,lastUpdate=Date.now(),pollMs=1000,pollTimer=null,batteryPresets=[],serverFormDirty=false,serverFormSaving=false,flowFormDirty=false,flowFormSaving=false,ledFormDirty=false,ledFormSaving=false,ledLiveTimer=null,ledSaveTimer=null,historyView='seconds',historyCapacityMax=600,runtimeHistoryMs=100,runtimeSecondCapacity=600,runtimeMinuteIntervalMs=60000,runtimeMinuteCapacity=360,runtimeInaAvg=1,browserHistoryDirty=false,browserHistorySaveTimer=null;const charts={},hist={l:[],ps:[],pb:[],pl:[],vs:[],vb:[],vl:[],is:[],ib:[],il:[]},browserHistory=[],SOLAR='#f59e0b',BAT='#10b981',BAT_DIS='#ef4444',COMBO='#a855f7',MUTED='#4a6080',WARN='#ef4444',LOAD='#38bdf8',BROWSER_HISTORY_KEY='solarMonitorBrowserHistoryV1',BROWSER_HISTORY_LIMIT=3600;
    const chartOpts={animation:false,responsive:true,maintainAspectRatio:false,interaction:{mode:'index',intersect:false},plugins:{legend:{labels:{color:'#4a6080',font:{family:'Courier New',size:10}}}},scales:{x:{display:true,grid:{display:false},ticks:{color:'#4a6080',font:{family:'Courier New',size:10},autoSkip:true,maxTicksLimit:8,maxRotation:0,minRotation:0}},y:{grid:{color:'#1e2d42'},ticks:{color:'#4a6080',font:{family:'Courier New',size:10}}}}};
    function mkDs(label,color,bg){return{label:label,data:[],borderColor:color,backgroundColor:bg,borderWidth:1.5,fill:true,tension:0,pointRadius:0}}
    function mkCfg(a,b,c,ca,cb,cc,bga,bgb,bgc){return{type:'line',data:{labels:[],datasets:[mkDs(a,ca,bga),mkDs(b,cb,bgb),mkDs(c,cc,bgc)]},options:chartOpts}}
    ...
    function historyIntervalForView(view){return view==='minutes'?runtimeMinuteIntervalMs:runtimeHistoryMs}
    function historyCapacityForView(view){return view==='minutes'?runtimeMinuteCapacity:runtimeSecondCapacity}
    function formatHistoryLabel(timestampMs,latestTimestampMs,view){const ageMs=Math.max(0,(Number(latestTimestampMs)||0)-(Number(timestampMs)||0));if(ageMs===0)return'now';if(view==='minutes')return'-'+(ageMs/60000).toFixed(ageMs>=600000?0:1)+'m';if(ageMs>=1000)return'-'+(ageMs/1000).toFixed(ageMs>=10000?0:1)+'s';return'-'+Math.round(ageMs)+'ms'}
    function visibleBrowserHistoryLimit(view){return view==='minutes'?Math.min(maxPts,runtimeMinuteCapacity):Math.min(maxPts,runtimeSecondCapacity)}
    function scheduleBrowserHistorySave(){if(browserHistorySaveTimer)clearTimeout(browserHistorySaveTimer);browserHistorySaveTimer=setTimeout(saveBrowserHistory,300)}
    function saveBrowserHistory(){browserHistorySaveTimer=null;if(!browserHistoryDirty)return;try{localStorage.setItem(BROWSER_HISTORY_KEY,JSON.stringify(browserHistory));browserHistoryDirty=false}catch(e){}}
    function loadBrowserHistory(){try{const raw=localStorage.getItem(BROWSER_HISTORY_KEY);if(!raw)return;const parsed=JSON.parse(raw);if(!Array.isArray(parsed))return;parsed.forEach(item=>{if(item&&Number.isFinite(Number(item.t)))browserHistory.push(item)});while(browserHistory.length>BROWSER_HISTORY_LIMIT)browserHistory.shift()}catch(e){}}
    function clearBrowserHistory(){browserHistory.length=0;browserHistoryDirty=true;saveBrowserHistory();renderHistoryFromBrowser('Browser history cleared')}
    function appendBrowserHistorySample(d){const sample={t:Number(d.timestamp_ms)||Date.now(),ps:Number(d.solar&&d.solar.power_mw)||0,pb:Number(d.battery_power_signed_mw)||0,pl:Number(d.load&&d.load.power_mw)||0,vs:Number(d.solar&&d.solar.voltage)||0,vb:Number(d.battery&&d.battery.voltage)||0,vl:Number(d.load&&d.load.voltage)||0,is:Number(d.solar&&d.solar.current_ma)||0,ib:Number(d.battery&&d.battery.current_ma)||0,il:Number(d.load&&d.load.current_ma)||0};const last=browserHistory.length?browserHistory[browserHistory.length-1]:null;if(last&&Math.abs(sample.t-last.t)<250)return;browserHistory.push(sample);while(browserHistory.length>BROWSER_HISTORY_LIMIT)browserHistory.shift();browserHistoryDirty=true;scheduleBrowserHistorySave()}
    function buildMinuteBrowserHistory(){const points=[];let currentBucket=-1,lastInBucket=null;browserHistory.forEach(sample=>{const bucket=Math.floor((Number(sample.t)||0)/60000);if(bucket!==currentBucket){if(lastInBucket)points.push(lastInBucket);currentBucket=bucket}lastInBucket=sample});if(lastInBucket)points.push(lastInBucket);return points}
    function renderHistoryFromBrowser(noteOverride){const source=historyView==='minutes'?buildMinuteBrowserHistory():browserHistory.slice(),limit=visibleBrowserHistoryLimit(historyView),points=source.slice(-limit),latestTimestampMs=points.length?(Number(points[points.length-1].t)||0):0,intervalMs=historyIntervalForView(historyView);resetHist();points.forEach(p=>appendPoint(formatHistoryLabel(p.t,latestTimestampMs,historyView),{ps:p.ps,pb:p.pb,pl:p.pl,vs:p.vs,vb:p.vb,vl:p.vl,is:p.is,ib:p.ib,il:p.il}));syncCharts();renderHistoryModeSummary(noteOverride||('History stored in this browser. Visible span ~'+formatHistorySpan(points.length*intervalMs)+' | INA avg x'+runtimeInaAvg))}
    function renderHistoryModeSummary(noteOverride){const intervalMs=historyIntervalForView(historyView),visiblePoints=visibleBrowserHistoryLimit(historyView),visibleSpanMs=visiblePoints*intervalMs;document.querySelectorAll('[data-history-view]').forEach(btn=>btn.classList.toggle('active',btn.dataset.historyView===historyView));document.getElementById('history-mode-note').textContent=historyView==='minutes'?'Minute view: browser stores 1 s live snapshots and shows the last sample each minute.':'Second view: browser stores raw live snapshots every report interval.';document.getElementById('history-source-note').textContent=noteOverride||('History stored in this browser. Visible span ~'+formatHistorySpan(visibleSpanMs)+' | INA avg x'+runtimeInaAvg)}
    function setHistoryView(view){historyView=view==='minutes'?'minutes':'seconds';renderHistoryFromBrowser()}
    ...
    function setPoll(ms){pollMs=Math.max(1000,Number(ms)||1000)}
    ...
    function renderRuntimeConfig(data){const reportMs=Math.max(200,Number(data.sample_interval_ms)||1000),sensorPollMs=Math.max(1,Number(data.sensor_poll_interval_ms)||1),historyMs=Math.max(10,Number(data.history_interval_ms)||100),secondsCap=Math.max(30,Number(data.history_seconds_capacity||data.history_capacity)||600),rawChartPts=Number(data.chart_point_limit)||300,inaAvg=Math.max(1,Number(data.ina_averaging_samples)||1),inaBusUs=Math.max(140,Number(data.ina_bus_conv_us)||140),inaShuntUs=Math.max(140,Number(data.ina_shunt_conv_us)||140),theoryHz=Number(data.ina_estimated_channel_hz)||0,measuredHz=Number(data.sensor_measured_hz)||0,regHex=data.ina_config_register_hex||'0x7007';historyCapacityMax=Math.max(secondsCap,360);const chartPts=clampChartPoints(rawChartPts),secondsSpanMs=Math.min(secondsCap,BROWSER_HISTORY_LIMIT)*reportMs,minuteSpanMs=360*60000;if(document.activeElement!==document.getElementById('cfg-sensor-poll'))document.getElementById('cfg-sensor-poll').value=sensorPollMs;if(document.activeElement!==document.getElementById('cfg-interval'))document.getElementById('cfg-interval').value=reportMs;if(document.activeElement!==document.getElementById('cfg-history-interval'))document.getElementById('cfg-history-interval').value=historyMs;if(document.activeElement!==document.getElementById('cfg-chart-points'))document.getElementById('cfg-chart-points').value=chartPts;document.getElementById('cfg-ina-avg').value=String(inaAvg);document.getElementById('cfg-ina-bus-us').value=String(inaBusUs);document.getElementById('cfg-ina-shunt-us').value=String(inaShuntUs);runtimeHistoryMs=reportMs;runtimeSecondCapacity=Math.min(BROWSER_HISTORY_LIMIT,secondsCap);runtimeMinuteIntervalMs=60000;runtimeMinuteCapacity=360;runtimeInaAvg=inaAvg;maxPts=chartPts;document.getElementById('interval-note').textContent='Poll '+sensorPollMs+' ms | Report '+reportMs+' ms | Browser seconds history ~'+formatHistorySpan(secondsSpanMs)+' | Browser minutes history ~'+formatHistorySpan(minuteSpanMs)+' | Chart '+chartPts+' pts';document.getElementById('runtime-ina-note').textContent='INA cfg '+regHex+' | avg x'+inaAvg+' | bus '+inaBusUs+' us | shunt '+inaShuntUs+' us | theory '+(theoryHz?theoryHz.toFixed(1):'0.0')+' Hz | measured '+(measuredHz?measuredHz.toFixed(1):'0.0')+' Hz';renderHistoryFromBrowser()}
    ...
    function syncOne(ch,keyA,keyB,keyC){if(!ch)return;ch.data.labels=hist.l.slice();ch.data.datasets[0].data=hist[keyA].slice();ch.data.datasets[1].data=hist[keyB].slice();ch.data.datasets[2].data=hist[keyC].slice();ch.update('none')}
    function syncCharts(){if(!chartReady)return;syncOne(charts.power,'ps','pb','pl');syncOne(charts.voltage,'vs','vb','vl');syncOne(charts.current,'is','ib','il')}
    function appendPoint(label,p){if(hist.l.length>=maxPts){Object.keys(hist).forEach(k=>hist[k].shift())}hist.l.push(label||'');hist.ps.push(Number(p.ps)||0);hist.pb.push(Number(p.pb)||0);hist.pl.push(Number(p.pl)||0);hist.vs.push(Number(p.vs)||0);hist.vb.push(Number(p.vb)||0);hist.vl.push(Number(p.vl)||0);hist.is.push(Number(p.is)||0);hist.ib.push(Number(p.ib)||0);hist.il.push(Number(p.il)||0)}
    function resetHist(){hist.l=[];hist.ps=[];hist.pb=[];hist.pl=[];hist.vs=[];hist.vb=[];hist.vl=[];hist.is=[];hist.ib=[];hist.il=[]}
    function waitForCharts(){const t=setInterval(()=>{if(typeof Chart!=='undefined'){clearInterval(t);charts.power=new Chart(document.getElementById('powerChart').getContext('2d'),mkCfg('Solar Power (mW)','Battery Power (mW)','Load Power (mW)',SOLAR,BAT,LOAD,'rgba(245,158,11,0.08)','rgba(16,185,129,0.08)','rgba(56,189,248,0.06)'));charts.voltage=new Chart(document.getElementById('voltageChart').getContext('2d'),mkCfg('Solar Voltage (V)','Battery Voltage (V)','Load Voltage (V)',SOLAR,BAT,LOAD,'rgba(245,158,11,0.08)','rgba(16,185,129,0.08)','rgba(56,189,248,0.06)'));charts.current=new Chart(document.getElementById('currentChart').getContext('2d'),mkCfg('Solar Current (mA)','Battery Current (mA)','Load Current (mA)',SOLAR,BAT,LOAD,'rgba(245,158,11,0.08)','rgba(16,185,129,0.08)','rgba(56,189,248,0.06)'));chartReady=true;renderHistoryFromBrowser()}},150)}
    ...
    window.addEventListener('load',()=>{
      ...
      waitForCharts();
      fetchConfig();
      renderHistoryFromBrowser('Loading browser history...');
      fetchData();
      document.addEventListener('visibilitychange',()=>{if(document.hidden)saveBrowserHistory();schedulePoll()});
      window.addEventListener('beforeunload',saveBrowserHistory);
      ...
      document.querySelectorAll('[data-history-view]').forEach(btn=>btn.onclick=()=>setHistoryView(btn.dataset.historyView));
...
void WebUi::handleHealth_() {
  ...
  appendRuntimeConfig(healthRoot, config, sensors_, historyBuffer_, minuteHistoryBuffer_);
...
void WebUi::handleStatus_() {
  ...
  appendRuntimeConfig(statusRoot, config, sensors_, historyBuffer_, minuteHistoryBuffer_);
...
void WebUi::handleConfig_() {
  ...
  appendRuntimeConfig(configRoot, config, sensors_, historyBuffer_, minuteHistoryBuffer_);
...
void WebUi::handleSaveRuntime_() {
  ...
  appendRuntimeConfig(root, wifiService_.config(), sensors_, historyBuffer_, minuteHistoryBuffer_);
