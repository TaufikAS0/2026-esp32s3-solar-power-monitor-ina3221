...
<body>
  ...
  <script>
    ...
    function displaySamplesForScale(points,scale,sourceMode){if(sourceUsesRaw(sourceMode))return points.slice();if(scale.id==='1min'||scale.id==='10min')return points.slice();return points.length>maxPts?downsampleBrowserSamples(points,maxPts):points.slice()}
