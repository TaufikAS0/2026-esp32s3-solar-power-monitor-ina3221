...
<body>
  ...
  <script>
    ...
    function displaySamplesForScale(points,scale,sourceMode){if(!sourceUsesRaw(sourceMode)){if(scale.id==='1min')return points.slice();return points.length>maxPts?downsampleBrowserSamples(points,maxPts):points.slice()}if(scale.id==='10s')return stableBucketSamples(points,10);return points.slice()}
