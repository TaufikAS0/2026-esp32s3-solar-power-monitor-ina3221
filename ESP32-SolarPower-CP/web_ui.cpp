...
<body>
  ...
  <script>
    let maxPts=300,chartReady=false,lastUpdate=Date.now(),pollMs=1000,pollTimer=null,batteryPresets=[],serverFormDirty=false,serverFormSaving=false,flowFormDirty=false,flowFormSaving=false,ledFormDirty=false,ledFormSaving=false,ledLiveTimer=null,ledSaveTimer=null,historyScale='1s',historyCapacityMax=600,runtimeReportMs=1000,runtimeRawHistoryCapacity=600,runtimeInaAvg=1,browserHistoryDirty=false,browserHistorySaveTimer=null;const charts={},browserHistory=[],SOLAR='#f59e0b',BAT='#10b981',BAT_DIS='#ef4444',COMBO='#a855f7',MUTED='#4a6080',WARN='#ef4444',LOAD='#38bdf8',BROWSER_HISTORY_KEY='solarMonitorBrowserHistoryV1',BROWSER_HISTORY_LIMIT=3600,HISTORY_SCALE_KEY='solarMonitorHistoryScaleV1',HISTORY_SCALES={s1:{id:'1s',label:'1s',gridMs:1000,windowMs:10000},s3:{id:'3s',label:'3s',gridMs:3000,windowMs:30000},s10:{id:'10s',label:'10s',gridMs:10000,windowMs:100000},m1:{id:'1min',label:'1min',gridMs:60000,windowMs:600000}};
    ...
    function createChartOptions(){return{animation:false,responsive:true,maintainAspectRatio:false,interaction:{mode:'nearest',intersect:false},plugins:{legend:{labels:{color:'#4a6080',font:{family:'Courier New',size:10}}}},scales:{x:{type:'linear',display:true,min:-10,max:0,grid:{display:true,color:'#1e2d42'},ticks:{color:'#4a6080',font:{family:'Courier New',size:10},stepSize:1,maxTicksLimit:11,maxRotation:0,minRotation:0,callback:value=>formatHistoryTick(value)}},y:{grid:{color:'#1e2d42'},ticks:{color:'#4a6080',font:{family:'Courier New',size:10}}}}}}
    function mkDs(label,color,bg){return{label:label,data:[],parsing:false,borderColor:color,backgroundColor:bg,borderWidth:1.5,fill:true,tension:0,pointRadius:0}}
    ...
    function activeHistoryScale(){if(historyScale==='3s')return HISTORY_SCALES.s3;if(historyScale==='10s')return HISTORY_SCALES.s10;if(historyScale==='1min')return HISTORY_SCALES.m1;return HISTORY_SCALES.s1}
    function rawBrowserHistorySpanMs(){return Math.min(runtimeRawHistoryCapacity,BROWSER_HISTORY_LIMIT)*runtimeReportMs}
    function visibleWindowPointEstimate(scale){return Math.max(1,Math.round(scale.windowMs/Math.max(1,runtimeReportMs)))}
    function saveHistoryScale(){try{localStorage.setItem(HISTORY_SCALE_KEY,historyScale)}catch(e){}}
    function loadHistoryScale(){try{const stored=localStorage.getItem(HISTORY_SCALE_KEY);if(stored==='1s'||stored==='3s'||stored==='10s'||stored==='1min')historyScale=stored}catch(e){}}
    ...
    function renderHistoryFromBrowser(noteOverride){const scale=activeHistoryScale(),source=browserHistory.slice();if(!source.length){syncCharts([],Date.now(),scale);renderHistoryModeSummary(noteOverride||'Waiting for live browser samples...');return}const latestTimestampMs=Number(source[source.length-1].t)||Date.now(),windowStartMs=latestTimestampMs-scale.windowMs;let visible=source.filter(p=>(Number(p.t)||0)>=windowStartMs);if(!visible.length)visible=source.slice(-1);const rawCount=visible.length;if(visible.length>maxPts)visible=downsampleBrowserSamples(visible,maxPts);syncCharts(visible,latestTimestampMs,scale);const note=noteOverride||('Grid '+scale.label+' | Window '+formatHistorySpan(scale.windowMs)+' | Raw '+rawCount+' pts'+(rawCount!==visible.length?(' -> chart '+visible.length+' pts'):'')+' | INA avg x'+runtimeInaAvg);renderHistoryModeSummary(note)}
    function renderHistoryModeSummary(noteOverride){const scale=activeHistoryScale(),rawSpanMs=rawBrowserHistorySpanMs();document.querySelectorAll('[data-history-scale]').forEach(btn=>btn.classList.toggle('active',btn.dataset.historyScale===historyScale));document.getElementById('history-mode-note').textContent='Vertical grid '+scale.label+' | Moving window '+formatHistorySpan(scale.windowMs)+' | About '+visibleWindowPointEstimate(scale)+' raw samples at current report rate.';document.getElementById('history-source-note').textContent=noteOverride||('Browser raw history ~'+formatHistorySpan(rawSpanMs)+' | Report '+runtimeReportMs+' ms | INA avg x'+runtimeInaAvg)}
    function setHistoryScale(view){historyScale=(view==='3s'||view==='10s'||view==='1min')?view:'1s';saveHistoryScale();renderHistoryFromBrowser()}
    ...
    function pointSeries(points,latestTimestampMs,key){return points.map(p=>({x:(Number(p.t)-latestTimestampMs)/1000,y:Number(p[key])||0}))}
    function syncOne(ch,points,latestTimestampMs,keyA,keyB,keyC,scale){if(!ch)return;ch.data.datasets[0].data=pointSeries(points,latestTimestampMs,keyA);ch.data.datasets[1].data=pointSeries(points,latestTimestampMs,keyB);ch.data.datasets[2].data=pointSeries(points,latestTimestampMs,keyC);ch.options.scales.x.min=-(scale.windowMs/1000);ch.options.scales.x.max=0;ch.options.scales.x.ticks.stepSize=scale.gridMs/1000;ch.update('none')}
    function syncCharts(points,latestTimestampMs,scale){if(!chartReady)return;syncOne(charts.power,points,latestTimestampMs,'ps','pb','pl',scale);syncOne(charts.voltage,points,latestTimestampMs,'vs','vb','vl',scale);syncOne(charts.current,points,latestTimestampMs,'is','ib','il',scale)}
