...
<body>
  ...
  <script>
    ...
    function visibleWindowPointEstimate(scale,mode){if(sourceUsesRaw(mode)||(!mode&&scale.source==='raw')){const hz=runtimeMeasuredHz>0?runtimeMeasuredHz:(1000/Math.max(1,runtimeSensorPollMs));return Math.max(1,Math.round(scale.windowMs*hz/1000))}return Math.max(1,Math.round(scale.windowMs/Math.max(1,runtimeReportMs)))}
    ...
    function sourceModeLabel(mode){if(mode==='raw-live')return'live burst';if(mode==='raw-cache')return'browser raw cache';return'browser archive'}
    ...
    function chooseHistorySource(scale,startMs,endMs){if(scale.source==='trend'){if(startMs>0&&endMs>startMs&&rawSourceCovers(startMs,endMs))return{points:rawHistory,mode:viewFollowLive?'raw-live':'raw-cache'};if(trendHistory.length)return{points:trendHistory,mode:'trend-archive'};if(rawHistory.length)return{points:rawHistory,mode:'raw-cache'};return{points:[],mode:'trend-archive'}}if(startMs===0&&endMs===0)return rawHistory.length?{points:rawHistory,mode:'raw-live'}:(trendHistory.length?{points:trendHistory,mode:'trend-archive'}:{points:[],mode:'raw-live'});if(rawSourceCovers(startMs,endMs))return{points:rawHistory,mode:viewFollowLive?'raw-live':'raw-cache'};if(trendHistory.length)return{points:trendHistory,mode:'trend-archive'};return rawHistory.length?{points:rawHistory,mode:'raw-cache'}:{points:[],mode:'raw-live'}}
